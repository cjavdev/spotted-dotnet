...
public class ShowListParamsTest : TestBase
{
    [Fact]
    ...
    {
        var parameters = new ShowListParams { };
    ...
    {
        ShowListParams parameters = new() { Limit = 10, Offset = 5 };

        var url = parameters.Url(new() { AccessToken = "My Access Token" });

        Assert.True(
            TestBase.UrisEqual(
                new Uri("https://api.spotify.com/v1/me/shows?limit=10&offset=5"),
                url
            )
        );
    }
