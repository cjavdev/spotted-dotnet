...
/// </summary>
public record class TrackRetrieveParams : ParamsBase
{
...
#pragma warning restore CS8618
    ...
    public static TrackRetrieveParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData
    )
    {
    ...
    public virtual bool Equals(TrackRetrieveParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/tracks/{0}", this.ID)
        )
        {
