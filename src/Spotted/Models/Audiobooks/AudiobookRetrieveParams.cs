...
/// </summary>
public record class AudiobookRetrieveParams : ParamsBase
{
...
#pragma warning restore CS8618
    ...
    public static AudiobookRetrieveParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData
    )
    {
    ...
    public virtual bool Equals(AudiobookRetrieveParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/audiobooks/{0}", this.ID)
        )
        {
