...
/// </summary>
public record class EpisodeRetrieveParams : ParamsBase
{
...
#pragma warning restore CS8618
    ...
    public static EpisodeRetrieveParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData
    )
    {
    ...
    public virtual bool Equals(EpisodeRetrieveParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/episodes/{0}", this.ID)
        )
        {
